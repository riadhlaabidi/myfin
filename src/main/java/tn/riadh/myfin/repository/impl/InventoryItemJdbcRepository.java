...
import tn.riadh.myfin.repository.InventoryItemRepository;
import tn.riadh.myfin.repository.mapper.InventoryItemMapper;
...
public class InventoryItemJdbcRepository implements InventoryItemRepository {
    ...
    public Optional<InventoryItem> findById(Long id) {
        ...
        List<InventoryItem> items = jdbcTemplate.query(sql, inventoryItemMapper, id);
        if (items.isEmpty()) {
    ...
    public Optional<InventoryItem> findByProductId(Long productId) {
        ...
        List<InventoryItem> items = jdbcTemplate.query(sql, inventoryItemMapper, productId);
        if (items.isEmpty()) {
            return Optional.empty();
        }
        return Optional.of(items.getFirst());
